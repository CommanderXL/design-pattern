...
(function (global) {
	...
	function request_script(chain_opts, script_obj, registry_item, onload, preload_this_script) {
      ...
      // preload_this_script = true
      // real_preloading = false
			...
			if (preload_this_script) {
				...
				// 如果是同域的话而且允许使用XHR请求, 那么就使用XHR preloading
			...
			// async属性也是异步加载,但是js被加载完后立马就会被执行，而不会考虑到执行的顺序
	...
	function create_sandbox() {
		...
		/*!START_DEBUG*/
		global_defaults[_Debug] = true; /*!END_DEBUG*/
		global_defaults[_BasePath] = "";

		// execute a script that has been preloaded already
		// 执行一个已经被预加载的script内容
		// script_obj是传入的script标签对象的属性
		// TODO: register_item是什么?
		...
		function do_script(chain_opts, script_obj, chain_group, preload_this_script) {
			...
			if (chain_opts[_AllowDuplicates] || registry_items.length == 0) {
				...
				// 开始请求js代码, script_obj表示这个script标签obj属性, register_item表示这个scipt标签的状态
		...
		function create_chain() {
			var chainedAPI,
				chain_opts = merge_objs(global_defaults, {}), 	// 链式调用的config属性
				chain = [],
				exec_cursor = 0,
			...
			function chain_script_executed(script_obj, chain_group) {
				...
				script_obj.ready = script_obj.finished = true;   	// 标志位
			...
			function advance_exec_cursor() {
				...
				if (exec_cursor == chain.length) {
					scripts_currently_loading = false; 	// 标志位
					group = false;
				}
			}
					...
					for (var i = 0; i < arguments.length; i++) {
						(function (script_obj, script_list) {
							...
							for (var j = 0; j < script_list.length; j++) {
								...
								// 如果 script_obj 是个字符串，即 $LAB.script('http://localhost:8080/a.js') 这种形式
								if (typeof script_obj == "string") script_obj = {
									src: script_obj
								};
								script_obj = merge_objs(script_obj, {
									ready: false,
									ready_cb: chain_script_ready,   // TODO:
									finished: false,
									finished_cb: chain_script_executed  
								});
								group.finished = false;  					// TODO: 
